...
def DFS(control_socket, data_socket, f, dirs, files, target):
    ...
    if target in files:
        ...
        send_command_and_check(control_socket, f, "PUT", "test_" + target)
        
        with open(TARGET_FILENAME, "rb") as f:
            data_bytes = f.read()

        header_bytes = len(data_bytes).to_bytes(HEADER_SIZE, byteorder='big')

        send_data(SERVER_PORT, header_bytes, data_bytes)
...
try:
    ...
    send_command_and_check(s, f, "GPRT", MY_PORT)
